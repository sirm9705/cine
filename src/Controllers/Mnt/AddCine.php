...
class AddCine extends PublicController
{
    ...
    public function run(): void
    {
        ...
        $this->processView();
        Renderer::render('mnt/addcine', $this->viewData);
    }
    ...
    private function procesarGet()
    {
        ...
        if ($this->viewData["mode"] !== "INS" && isset($_GET["id"])) {
            ...
            $tmpProducto = DaoCines::getById($this->viewData["ID"]);
    ...
    private function CampoVacio($Campo)
    {
        if (Validators::IsEmpty($this->viewData["$Campo"])) {
            $this->viewData["error_$Campo"][]
                = "El $Campo del cine es requerido";
            $this->hasErrors = true;
        }
    }
            ...
            switch ($this->viewData["mode"]) {
                case 'INS':
                    ...
                    if ($result) {
                        \Utilities\Site::redirectToWithMsg(
                            "index.php?page=mnt_index",
                            "el cine se ha Guardado Satisfactoriamente!"
                        );
                    }
                    break;
                case 'UPD':
                    ...
                    if ($result) {
                        \Utilities\Site::redirectToWithMsg(
                            "index.php?page=mnt_index",
                            "el cine se ha Actualizado Satisfactoriamente"
                        );
                    }
                    break;
                case 'DEL':
                    $result = DaoCines::delete(
                        intval($this->viewData["ID"])
                    );
                    if ($result) {
                        \Utilities\Site::redirectToWithMsg(
                            "index.php?page=mnt_cines",
                            "el cine se ha Eliminado Satisfactoriamente"
                        );
                    }
                    break;
            }
